...
" To use these shortcuts: first activate command line with ':', then input 
" the short alias, e.g., then press <space>, the alias will be expanded to
...
command! -nargs=0 StartVline call jobstart(printf("sbcl --load %s/vlime/lisp/start-vlime.lisp", g:package_home))
...
" Do not use version control tool to list files under a directory 
" since submodules are not searched by default
...
if g:is_mac
  ...
  augroup grammous_map
    ...
    autocmd FileType makrdown nmap <buffer> <c-p> <Plug>(grammarous-move-to-previous-error)
  augroup END
endif
